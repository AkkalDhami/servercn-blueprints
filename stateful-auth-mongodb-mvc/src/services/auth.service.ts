...
import { generateTokenAndHashedToken } from "../helpers/token.helpers";
...
export class AuthService {
  ...
  static async handleToken(
    ...
    if (context.userAgent) {
      ...
      if (isNewDevice) {
        ...
        <p> 
          ...
          <strong>Time:</strong> ${new Date().toLocaleDateString()}<br/>
        </p>
    ...
    const { hashedToken, token } = generateTokenAndHashedToken(
      user._id.toString()
    );
